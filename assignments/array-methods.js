...
let ticketPriceTotal = runners.reduce((a, c) => a += c.donation, 0);
...
// Problem 1
// The fun run wants to give a special mention to all companies that donated more than 100. Log a list of the companies and and their donation that are 100 or more.
let bigDonators = [];
runners.filter(runner => runner.donation >= 100).forEach((runner) => bigDonators.push({ company: runner.company_name, donation: runner.donation }));
console.log(bigDonators);

// Problem 2
// The fun run wants to e-mail a thank you to all the representatives that attended but needs a list of their e-mail addresses.
let emails = [];
...
let lnames = [];
runners.forEach((runner) => lnames.push(runner.last_name));
console.log(lnames);